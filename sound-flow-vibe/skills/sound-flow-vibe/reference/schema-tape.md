...
| `wobble` | `depthMs`, `rateHz`, `mix?` | 0–12 ms, 0.1–8 Hz, 0–1 (=1) | Pitch instability off a modulated 15 ms delay. Slow+shallow (2 ms / 0.5 Hz) = warped-record wow; fast+shallow (1–2 ms / 6 Hz) = flutter; deep (8–12 ms) = seasick. LFO phase starts at t0 — deterministic. `mix` (shared with STOMP via `lib/fx-chain.mjs`) defaults to 1 = fully wet, which is the graph TAPE has always built; below 1 the dry signal rejoins and the wobble reads as a chorus rather than a warp. |
